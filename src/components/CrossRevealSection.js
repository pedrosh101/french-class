...
function CrossRevealSection({
  ...
  // animate the img the opposite way at the same time
  const imageRef = useRef();
  // specify the point we want our animation to start
  const triggerRef = useRef();
  // target the person contaier
  const personRef = useRef();
  // target the quote container
  const quoteRef = useRef();

  useEffect(() => {
    //always end tl with Tween
    ...
    crossRevealTween
      ...
      .fromTo(personRef.current, { opacity: 0 }, { opacity: 1 }, 0)
      // fade in the quote
  ...
  return (
    <StyledCrossRevealContainer ref={triggerRef}>
      ...
      <div className="landscape__wrapper">
        <p className="quote__sentence" ref={quoteRef}>
          {sentenceOne}
          <br />
          {sentenceTwo}
        </p>
